...
class JSGObjectExpr(jsgParserVisitor, PythonGeneratorElement):
    ...
    def map_as_python(self, name: str) -> str:
        if self._map_name_type is not None:
            ...
            name_filter = "{}_name_filter = {}".format(indent1n, filtr)
        else:
            name_filter = ""
        if self._map_valuetype is not None:
            ...
            typ = "jsg.AnyType"
        value_type = "{}_value_type = {}".format(indent1n, typ)
    ...
    def _gen_members_list(self) -> str:
        if self._map_valuetype:
            ...
            return indent0.join("'{}': {}".format(mk, mt) for mk, mt in self.members_entries())
    ...
    def signatures(self, _: bool=False) -> List[str]:
        if self._map_valuetype:
            ...
            if len(self._choices) > 1:
                pair_signatures = 'opts_: typing.Union[{}] = None'.format(pair_signatures)
            else:
                pair_signatures = '{}: {} = None'.format(self._choices[0], pair_signatures)
    ...
    def initializers(self, prefix: Optional[str] = None) -> List[str]:
        ...
        if self._map_valuetype:
            ...
            if not prefix:
                prefix = 'opts_'
            # Overall test
            rval = ['if {} is not None:'.format(prefix)]

            # First choice option
            choice = self._choices[0]
            rval.append('{}if isinstance({}, {}):'.format(t(1), prefix, choice))
            ...
            for choice in self._choices[1:]:
                if is_valid_python(choice):
                    rval.append('{}elif isinstance({}, {}):'.format(t(1), prefix, choice))
                    choice_ref = self._context.reference(choice)
                    choice_inits = []
